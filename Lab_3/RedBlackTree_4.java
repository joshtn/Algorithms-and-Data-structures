 ...
 * @Author Josh
 * @Made 29/9/2019
 * @ProblemSolved Red black tree implementation used to compare with BST.
 * @HowToUse run main, change value of how many strings you want to read.
 * @BasedOn Algoritms 4th edition and lectures.
 */
public class RedBlackTree_4<Key extends Comparable<Key>, Value> {
    ...
    private Node put(Node h, Key key, Value val) {
        if (h == null) return new Node(key, val, RED, 1); // insert bottom, color red -------

        // find home for the value at correct place.
        ...
        if (isRed(h.right) && !isRed(h.left)) h = rotateLeft(h); // right child red, left child black
        if (isRed(h.left) && isRed(h.left.left)) h = rotateRight(h); // left child red, left left child red.
        if (isRed(h.left) && isRed(h.right)) flipColors(h); // both children red, swap color
    ...
    public static void FrequencyCounter(int minLength, int n) {
        ...
        RedBlackTree_4<String, Integer> sT = new RedBlackTree_4<>();
